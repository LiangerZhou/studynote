...
commonUtils.isNativePage()?showAppToast("Sending","loading"):show(qs("js_loading"));
}
...
return e&&e.length>maxLikeCommentWord?void alert("想法不可以超过%s字".replace("%s",maxLikeCommentWord)):void alert("Network unavailable. Try again later.");
}
...
alert(" Wow canceled and comment deleted.");
},20):showToast("Canceled"),2===showType&&isShow(el_alikeComment)&&(hide(el_alikeComment),
...
prompted){
case 0:
qs("educate_title").innerHTML="Added to Top Stories",show(el_likeEducate),show(educate_btn2),
prompted=1;
break;
...
if(n||t.isAndroid){
var h=w("js_preview_reward_author");
h&&(h.style.display="block");
var p=w("js_preview_reward_author_wording");
r.reward_wording&&p&&(p.innerText=r.reward_wording,p.style.display="block");
var l=w("js_preview_reward_author_link");
l&&s.on(l,"tap",function(e){
e.preventDefault(),a("Unable to do this in preview");
});
}
if(n){
var c=w("js_preview_reward_author_avatar"),v=w("js_preview_reward_author_head");
r.reward_author_head&&c&&v&&(v.setAttribute("src",r.reward_author_head),c.style.display="block");
